...
if (!empty($_FILES)){
	...
	if (move_uploaded_file($_FILES['logo']['tmp_name'], $fichero_subido)) {
		$query = "UPDATE head SET url='{$fichero_subido}' WHERE id=1";
		$rs = $db->Execute($query);
	}else{
		echo "no se puede subir";
	}
}
$query = "SELECT url FROM head WHERE id=1";
...
<html>	
	...
	<body id="body" class="indigo darken-4 cyan-text text-lighten-2">
		...
		<div class=" indigo darken-1">
			<div class="container">
				<div class="row">
					<div class="col s12 m12 l12 xl12"><h4><i class="Small material-icons left" style="line-height: inherit;">build</i>Head</h4></div>
				</div>			
			</div>
		</div>
		<!--head-->
